...
## quelllo che ho usato
raup_crick_abundance2 = function(datalist, classic_metric=FALSE, split_ties=TRUE, reps=9999, set_all_species_equal=FALSE, 
                                 as.distance.matrix=TRUE, report_similarity=FALSE, use.cores = 4){
  ...
  null_bray_curtis <- foreach(i = 1:reps, .packages = c("dplyr", "magrittr", "purrr", "vegan")) %dopar% {
    # Matrice vuota da riempire per ciascun sito
    com_shuffle_tmp <- matrix(0, nrow = n_sites, ncol = gamma)
    
    for (x in 1:n_sites) {
      richness_x <- richness_per_sample[x]
      count_x <- counts_per_sample[x]
      
      # STEP 1: Selezione ASV casuali in base a occur
      ASV_inds <- sample(seq_gamma, size = richness_x, replace = FALSE, prob = occur)
      
      # STEP 2: Assegna abbondanze ad ASV selezionati
      sample_probs <- abundance[ASV_inds]
      if (sum(sample_probs) == 0) {
        sample_probs <- NULL  # usa probabilità uniformi se tutti zero
      }
      
      sampled_counts <- sample(ASV_inds,
                               size = (count_x - richness_x),
                               replace = TRUE,
                               prob = sample_probs) %>%
        table() %>%
        ...
        rename("ASVs" = ".") %>%
        mutate(ASVs = as.numeric(ASVs)) %>%
        filter(ASVs %in% seq_gamma)  # sicurezza
      
      # Imposta presenza iniziale (1 per ogni ASV selezionato)
      com_shuffle_tmp[x, ASV_inds] <- 1
      
      # Aggiungi le abbondanze
      if (nrow(sampled_counts) > 0) {
        com_shuffle_tmp[x, sampled_counts$ASVs] <- com_shuffle_tmp[x, sampled_counts$ASVs] + sampled_counts$Freq
      }
    }
    
    # Calcola distanza Bray-Curtis su questa matrice randomizzata
    vegan::vegdist(com_shuffle_tmp) %>%
      as.matrix()
  }
...
rc_FL <- datalist %>% 
  ...
  raup_crick_abundance2(., classic_metric = F, split_ties = T, reps = bootstrap_reps, set_all_species_equal = F,
                       as.distance.matrix = F, report_similarity = F, use.cores = parallel_cores)
head(rc_FL)
write.csv(rc_FL, "output/Community_Mechanisms/Raup_Crick_Prok.csv")
