##--> da provare https://github.com/emblab-westlake/MbioAssy
...
wd="~/project/shared1/atlantic_sea4blue/16S_Illumina/Lapo_18May"

library(microbiome)
library(dplyr)
library(phyloseq)

 #### uso quelli prodotti nello script analisi_16S_atlantico

# the OTU table file (Tab delimited txt file)
com.file="Data/Processed/Prok/Full_Prok_Count.tsv"

# the phylogenetic tree file
tree.file="Data/Trees/Prok/Prok_Combined.tree"

# the classification (taxonomy) information
clas.file="Data/Taxonomy/Prok_Taxonomy.tsv"

 

##### questi li faccio giù in base a sto file
meta<-read.table("Data/Meta_Data/Prok/Meta_Data.tsv",header = T)
...
save.wd="iCAMP_output2"
if(!dir.exists(save.wd)){dir.create(save.wd)}

# 2 # key parameter setting
prefix="sea4blue"  # prefix of the output file names. usually use a project ID.
...
treat <- meta[, c("Sample_ID", "Zona")]
rownames(treat) <- treat$Sample_ID
treat$Sample_ID <- NULL

#  read.table(treat.file, header = TRUE, sep = "\t", row.names = 1,
#                 as.is = TRUE, stringsAsFactors = FALSE, comment.char = "",
#                 check.names = FALSE)

env=meta 
##l'avevo già fatto, quindi ho rifatto il file env (che non trovo più in base alle variabili usate)
folder_path <- "iCAMP_output/"  # <-- cambia con il tuo path
files <- list.files(folder_path)

# estrai i nomi base (prima del primo ".")
var_names <- unique(sub("\\..*", "", files))

# mostra le variabili trovate
print(var_names)

# seleziona solo le colonne di env che corrispondono a quei nomi
env_selected <- env[, colnames(env) %in% var_names]



rownames(env) <-env_selected
head(env)
env$Sample_ID <- NULL
#  read.table(env.file, header = TRUE, sep = "\t", row.names = 1,
#               as.is = TRUE, stringsAsFactors = FALSE, comment.char = "",
#               check.names = FALSE) 
...
# here set as 5 just for the small example dataset. For real data, usually try 12 to 48.
