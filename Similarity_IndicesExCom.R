...
fast.cophenetic.phylo <- function(x, nblocks=1000, use.cores=4) {
  ...
  my.node_paths <- furrr::future_map(seq(1, ape::Ntip(x)), function (tip) {ape::nodepath(x, from=tip, to = 1)})
  ...
  split.sums <- furrr::future_map(my.split, function(g) {node.ages[my.combs[g,1]]+node.ages[my.combs[g,2]]});
  split.mrca <- furrr::future_map2_int(my.combs[,1], my.combs[,2], function(a,b) {max(intersect(my.node_paths[[a]], my.node_paths[[b]]))});
